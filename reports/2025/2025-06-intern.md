...
| [issue1925](https://github.com/openeuler-riscv/oerv-team/issues/1925) | 24.03SP2 lpi4a 镜像测试，确认该版本镜像在板卡上的启动情况 | N/A | 是 | N/A |
...
本月继续沿着双周报和会议支持往前推进，预热、主持、纪要、排版和会后分发已经能连起来做，线下会议记录也能顺着接到公开稿整理这一步。

### 外部可见交付

- 完成 `OERV 双周报：022 期 - 20250615` 和 `OERV 双周报：023 期 - 20250630` 两期推送，继续负责会前预热、双周会主持、会议纪要整理、正文提炼、排版和群内同步。[022](https://mp.weixin.qq.com/s/19ueoiSPz9s3z0MMsJrqrQ) [023](https://mp.weixin.qq.com/s/gSEHjhiTCLbyTRb3WoOfjA)
- 参与会议签到与记录志愿工作，补齐线下会议的基础支持，并把会议概要及时整理出来，方便后续双周报和回顾内容衔接。

### 正在进行的尝试/未来的计划

- 继续把双周报相关流程做细，尤其是会议记录转公开稿这一步，尽量把后续排版和校对前置理顺。
...
| [修复 master 的 promu，处理当前分支上的构建问题 · Issue #1877 · openeuler-riscv/oerv-team](https://github.com/openeuler-riscv/oerv-team/issues/1877) | 修复 master 的 promu，处理当前分支上的构建问题                       | https://gitee.com/src-openeuler/promu/pulls/26               | 任务完成，已关闭                   |                               |
...
| [修复 master 的 containernetworking-plugins，验证补丁在 oe2509 上的构建可用性 · Issue #1892 · openeuler-riscv/oerv-team](https://github.com/openeuler-riscv/oerv-team/issues/1892) | 修复 master 的 containernetworking-plugins，验证补丁在 oe2509 上的构建可用性 | https://gitee.com/src-openeuler/containernetworking-plugins/pulls/55 | 未关闭，社区没有review             |                               |
| [[Bonus\] 让 ccb 可以接受用户指定的本地镜像或者 tarball · Issue #1894 · openeuler-riscv/oerv-team](https://github.com/openeuler-riscv/oerv-team/issues/1894) | 加强 ccb 的功能，使本地镜像和 tarball 构建路径可用   | https://gitee.com/src-openeuler/ccb/pulls/10                 | 未关闭，社区没有review             |                               |
...
| [让 ccb 支持交叉编译 · Issue #1911 · openeuler-riscv/oerv-team](https://github.com/openeuler-riscv/oerv-team/issues/1911) | 加强 ccb 的功能，补上跨架构构建这条路径       | https://gitee.com/src-openeuler/ccb/pulls/10                 | 未关闭，社区没有review             |                               |
...
| [记录第一次修复的包：promu](https://www.cnblogs.com/dayq/p/18960558) | 博客园博客，记录第一次修包的流程和关键问题          |                                                              |                                    |                               |
...
| 关联 issue 链接 | Issue 类型和简介（如修复xxx，升级xxx） | 外部 PR 或相关工程链接 | 是否关闭 issue 及未关闭 issue 原因 | mentor 评价（本月可以没有） |
| --------------- | -------------------------------------- | ---------------------- | ---------------------------------- | -------------------------- |
| xxHash模块提交RVV代码|优化 xxHash 模块在 RISC-V 平台的表现，并把 RVV 优化直接送回上游| https://github.com/Cyan4973/xxHash/pull/1043| 已合并| |
| 为xxHash提交RISCV CI| 添加 RISC-V + RVV 相关 CI 测试| https://github.com/Cyan4973/xxHash/pull/1049 | 未合并 ||
| [#1842](https://github.com/openeuler-riscv/oerv-team/issues/1842)| 修复 OE 的 Deepin Desktop Environment 相关包，逐步清理 DDE 依赖链上的构建问题| [deepin-pw-check](https://gitee.com/src-openeuler/deepin-pw-check/pulls/22) [dtkcore](https://gitee.com/src-openeuler/dtkcore/pulls/28) | 已合并|无|
| [#1842](https://github.com/openeuler-riscv/oerv-team/issues/1842)| 修复 OE 的 Deepin Desktop Environment 相关包，逐步清理 DDE 依赖链上的构建问题| [dtkwidget](https://gitee.com/src-openeuler/dtkwidget/pulls/29) | 未合并(构建时部分包无法满足条件)|无|
| [#1919](https://github.com/openeuler-riscv/oerv-team/issues/1919) | 修复 kubekey 的 CGO_ENABLE 问题，处理 Go 构建链上的兼容性问题 | https://gitee.com/src-openeuler/kubekey/pulls/30 | 未关闭(PR未review) | 无|
...
| 关联 issue 链接 | Issue 类型和简介（如修复xxx，升级xxx） | 外部 PR 或相关工程链接 | 是否关闭 issue 及未关闭 issue 原因 | mentor 评价（本月可以没有） |
| --------------- | -------------------------------------- | ---------------------- | ---------------------------------- | -------------------------- |
|https://github.com/openeuler-riscv/oerv-team/issues/902 | 调查 bug 原因，继续跟踪上游反馈和本地复现情况 | https://bugzilla.mozilla.org/show_bug.cgi?id=1972506 | 未关闭，已提交 Bugzilla，但是还没得到解决，目前仍在调查中 |
|https://github.com/openeuler-riscv/oerv-team/issues/904 | 调查 bug 原因，继续跟踪上游反馈和本地复现情况 | 无 | 未关闭，查到 java 导致的问题，但是还无法在本地成功修复此 bug 仍在调查中|
...
- 围绕 Ethgent 以太网-串口测试板完成了一轮较大规模的原理图重构，主控从 `ESP32` 切换到 `ESP32P4` 后，重新梳理了主控外围器件设计，先把 DEMO 版本的整体原理图迁移到新的主控方案上。
- 更新 `HR911130A` RJ45 接口方案，改用 `R-RJ45R08P-C000` 和 `G2406S`，并重新完成这一部分的电路设计与 PCB layout，使接口选型和 BOM 成本更贴合后续打样需求。
- 协助设计 `Micro SD` 多路切换方案，同时补入一路千兆以太网接口，并继续完善 `ESP32P4` 外围原理图。这个部分关系到测试板后续对多路存储和网络链路的支持能力。
- 完成 `RTL8367RB-VB-CG` 与 `LAN8720` 的原理图重构，并补齐 `idc1.27mm` 接口封装绘制，给交换机、以太网 PHY 和外部接口之间的连接关系打好基础。
- 重构交换机 `VDDL 1v1` 电源原理图和 RJ45 接口原理图，继续收敛交换机供电、网络口和外围器件之间的设计细节。
- 增加和修改部分器件封装，绘制 `1220` 电池封装，并调整部分电阻选型，配合整体 BOM 和可制造性优化。
- 完成 `CSI` 接口原理图和施密特触发器电路重构，补齐测试板上图像接口、信号整形等外围功能模块。
- 推进交换机和 `ESP32P4` 外围器件部分 PCB layout，给后续整板布局、走线检查和打样验证留下可继续迭代的基础。
- 形成基于 `ESP32P4` 主控的全功能测试电路原理图，并完成 DEMO 版本新增外围硬件功能实现，后续可以围绕这版原理图继续做 layout 收敛和板级验证。

### 正在进行的尝试/未来的计划

- 继续完善 Ethgent 测试板的 PCB layout 和外围器件细节，重点收敛交换机、电源、RJ45、CSI、Micro SD 多路切换和 `ESP32P4` 外围器件之间的连接关系。
- 继续做原理图改进和可制造性优化，后续围绕关键器件 layout 方案、BOM 成本和打样可验证性继续推进。
...
- 推进主线分支滚动更新工程，在 `openEuler:Factory:selfbuild` 中将最小集扩展到 200+ 软件包。
- 本轮新增范围仍然以核心软件为主，同时覆盖编程语言、构建实用工具和系统支撑组件。代表性软件包包括 `icu`、`boost`、`procps`、`cryptsetup`、`corosync`、`llvm/llvm20`、`python-pip`、`ruby/ruby3.4`、`meson`、`ninja`、`dbus-1`、`glib2`、`curl`、`jsoncpp`、`systemtap`、`shadow` 和 `hostname` 等。
- 跟进内核升级与维护，Kernel 6.12 已合入主线。
- 维护构建系统基础设施，新增构建机和构建项目，并在社区层面围绕 multibuild 用法和构建系统改造需求进行交流。
- 继续推进 RuyiOS 方向，围绕软件包组成、构建系统能力和最小可启动镜像进行整理。当前重点包括最小可启动镜像演进、多版本并存能力以及更高效的构建路径。
- 在 TC 例会中同步 master 工程近期状态并提出待改进问题，同时推动 OEEP0023 草案讨论，用于规范化管理大型套组软件。

### 社区协作与会议

本月的社区协作继续围绕滚动更新工程、多版本管理和构建系统能力展开。`2025-06-05` 与 `2025-06-19` 的 SIG-RISC-V 双周例会继续例行分享滚动更新进展，`2025-06-06` 的 Kernel 例会跟进内核维护状态，`2025-06-10` 的 Compiler 例会则继续处理 JDK 升级遗留问题。`2025-06-11` 的 TC 会议讨论二进制提交草案，`2025-06-13` 与 `2025-06-27` 两次 RM 会议继续关注跨分支版本维护和发布管理问题，`2025-06-18` 的 EPKG 例会用于同步软件包日常进度。

围绕工程规范和构建系统改造，本月的讨论逐步收敛到更具体的方案上。`2025-06-20` 的 CICD 例会围绕 EulerMaker 优化进行沟通，`2025-06-25` 的 TC 例会提交 OEEP0023 草案并同步 master 工程状态，`2025-06-27` 的 CICD 例会进一步交流 multibuild 用法和改造需求，明确下一步推进要点。另在 `2025-06-16` 的 Infra 例会中，继续讨论自动升级相关议题，为后续构建系统基础设施改进做准备。

### 正在进行的尝试/未来的计划

- 继续补齐软件包和依赖关系，提升滚动分支的构建成功率和功能完整度。
- 针对 EulerMaker 暴露的问题继续与 CICD SIG 沟通。
- 推进 RuyiOS 骨架和配套基础设施支撑。

### 产出文档

- 滚动更新构建状态（用于 `2025-06-25` TC 会议同步）
- Multibuild 用法（用于 `2025-06-27` CICD 会议交流）
