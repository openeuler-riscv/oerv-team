...
| 关联 issue 链接 | Issue 类型和简介（如修复xxx，升级xxx） | 外部 PR 或相关工程链接 | 是否关闭 issue 及未关闭 issue 原因 | mentor 评价（本月可以没有） |
| --------------- | -------------------------------------- | ---------------------- | ---------------------------------- | -------------------------- |
| N/A | ThreadX 的 RISC-V 支持，补齐架构侧基础支持代码 | [threadx-riscv](https://github.com/eclipse-threadx/threadx/pull/416) | N/A | N/A |
| N/A | MCS 文档，整理关键步骤和环境说明 | N/A | N/A | N/A |
...
### 感悟思考
- 这个月找到一个比较有意思的RISC-V hypervisor项目salus，正在学习，希望能在近期进行分享0
- 这个月也在学习赵熙老师介绍的基于Penglai的图像识别项目
### 外部可见交付
| 关联 issue 链接 | Issue 类型和简介（如修复xxx，升级xxx） | 外部 PR 或相关工程链接 | 是否关闭 issue 及未关闭 issue 原因 | mentor 评价（本月可以没有） |
| --------------- | -------------------------------------- | ---------------------- | ---------------------------------- | -------------------------- |
| [调研penglai TEE并进行分享](https://github.com/openeuler-riscv/oerv-team/issues/1386) | https://www.bilibili.com/video/BV1VCAaeyEw7 | N/A | N/A | N/A |
### 正在进行的尝试/未来的计划
- 学习基于RISC-V的嵌入式虚拟化
- 学习基于Penglai的嵌入式AI应用
...
| [#1357](https://github.com/openeuler-riscv/oerv-team/issues/1357) | 总结文档，整理测试步骤、结论和问题记录                               | [pulls/52](https://github.com/openeuler-riscv/oerv-qa/pull/52) （此pr是根据社区回复进行的验证，文档更新） | 已关闭                             |                             |     |                             |
...
| 关联 issue 链接                                              | Issue 类型和简介（如修复xxx，升级xxx） | 外部 PR 或相关工程链接                                       | 是否关闭 issue 及未关闭 issue 原因 | mentor 评价（本月可以没有） |
| ------------------------------------------------------------ | -------------------------------------- | ------------------------------------------------------------ | ---------------------------------- | --------------------------- |
| [1042](https://github.com/openeuler-riscv/oerv-team/issues/1042) | mugen_LLVM_2409 相关用例修复                        |                                                              | 是                                 |                             |
| [1050](https://github.com/openeuler-riscv/oerv-team/issues/1050) | mugen_LLVM_2409 相关失败项处理                        |                                                              | 是                                 |                             |
| [1328](https://github.com/openeuler-riscv/oerv-team/issues/1328) | mugen_LLVM_2409 相关问题收敛                        |                                                              | 是                                 |                             |
...
这个月主要远程配合做 DeepSeek 和大模型软件生态栈在 RISC-V Linux 上的可行性调研。因为我原来更熟悉 CUDA 算子和运行时，比较自然的切入点其实是算子实现、kernel 调度和编译优化；但当时 RISC-V 上的 GPU 硬件与 ROCm 运行环境还处在摸底阶段，上层优化缺少稳定的实验基线，所以这次先把 CUDA 生态当成参照系来拆：上层是模型框架和推理服务，中间是算子库、图编译和通信库，再往下是 GPU runtime、驱动和内核侧资源管理。受限于远程实习和本地没有完整板卡/GPU 环境，我这边更多是结合资料、公开线索和老师同学同步的环境信息做研究性分析；这样看 ROCm 不是简单替换一个后端的问题，它更像是在新架构上重新确认一整套 Linux 图形计算与 AI 运行时生态是否能衔接起来。

### 外部可见交付

这部分工作更多是先把资料和线索看清楚，没有形成可以直接合入的 PR。当前阶段我主要先把问题放回 Linux 图形计算栈和开源 AI 运行时生态中理解，把大模型运行要经过的软件栈层次捋出来，看哪些环节会影响 RISC-V 上的大模型软件支持。

- 配合梳理 DeepSeek 相关推理链路在 RISC-V 上可能依赖的组件，包括 Python/模型框架、HIP/ROCm 适配层、rocBLAS/MIOpen/Composable Kernel/rocMLIR 这类算子与编译组件，以及 runtime、驱动和内核接口对设备识别、任务提交和数据流转的支撑关系。调研时重点不是只看包能不能装上，而是理解上层模型运行如何依赖这些基础生态环节。
- 从 CUDA 经验出发，对比了 CUDA closed stack 和 ROCm 开源栈在研究方法上的差异。CUDA 侧很多机制会被稳定的厂商工具链包装起来，ROCm 在 RISC-V 上则更容易把用户态库、编译选项、内核驱动和硬件平台的边界暴露出来；这对理解开源 AI 计算生态在新架构上的支持条件比较有帮助。
- 初步整理公开演示和社区线索的核验思路，把“已经打包”“能输出设备信息”和“能稳定跑推理/带宽测试”区分开。这个区分后面很重要，因为只看到 `rocminfo` 或仓库包并不等于完整计算链路已经跑通，还需要从 OpenCL/HSA、带宽测试和实际推理样例这些层次继续理解生态支持的完整度。

### 正在进行的尝试/未来的计划

- 继续围绕 ROCm 在 RISC-V 上的大模型软件生态做资料整理，把 DeepSeek 推理链路拆成模型框架、算子库、编译组件、runtime、驱动和内核接口几个层次来理解。
- 后续重点阅读 ROCm runtime 与 Linux 图形计算栈的衔接关系，先把上层算子和编译优化所依赖的基础运行条件理顺。
...
| | LLVM 平行宇宙修复 glew，完成对应包在 LLVM 构建链下的适配 | https://gitee.com/src-openeuler/glew/pulls/25 |
| | LLVM 平行宇宙修复 procmail，完成对应包在 LLVM 构建链下的适配 | https://gitee.com/src-openeuler/procmail/pulls/17 |
...
| [#1666](https://github.com/openeuler-riscv/oerv-team/issues/1666) | 镜像发布页修复与改进，处理展示和交互上的具体问题 | https://github.com/openeuler-riscv/imagepub-web/pull/1 ; https://github.com/openeuler-riscv/imagepub-web/pull/5; https://github.com/openeuler-riscv/imagepub-web/pull/6; https://github.com/openeuler-riscv/imagepub-web/pull/7; https://github.com/openeuler-riscv/imagepub-web/pull/8 | 是 |  |
...
| [#1666](https://github.com/openeuler-riscv/oerv-team/issues/1676) | 镜像发布页前端 bug 修复，定位并修正了对应实现问题，收掉页面行为上的明显错误 | https://github.com/openeuler-riscv/imagepub-web/pull/14; https://github.com/openeuler-riscv/imagepub-web/pull/16 | 是 |  |
| [#1666](https://github.com/openeuler-riscv/oerv-team/issues/1694) | 镜像发布页修复与改进，处理展示和交互上的具体问题 | https://github.com/openeuler-riscv/imagepub-web/pull/18 | 否，还未来得及合并PR |  |
...
- 参与 `OERV 双周报：015 期` 的发布协同，对双周内的社区动态、技术进展和活动信息做基础整理，保持双周报标题、栏目和发布时间的连续性。
- 配合维护 OERV 宣发稿件的发布节奏，对当月待发布内容做简单归类，把常规双周报和专题稿区分开，便于后续继续按固定节奏推进。

### 正在进行的尝试/未来的计划

- 继续熟悉 OERV 公众号稿件从内容收集、标题整理到发布前核对的流程，先把常规双周报这条线稳定下来。
